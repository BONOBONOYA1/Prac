...
public class MemoService {
   ...
   public Long update(Long id, MemoRequestDto requestDto) {
        Memo memo = memoRepository.findById(id).orElseThrow(     //수정할 데이터베이스가 있는지 없는지 확인작업이 필요함.
                () -> new IllegalArgumentException("아이디가 존재하지 않습니다.")  //IllegalArgumentException 메서드가 허가되지 않거나 부적절한 argument를 받았을 경우에 던져지는/던질 수 있는 예외입니다.
       ); //orElseThrow예외 처리
        memo.update(requestDto);//가지고 온 메모에 있는 값을 변경해줄 코드. update메소드를 사용해서 변경. 변경된 값은 client 에서 보내준 rquestDto안에 들어가 있는 값으로 수정한다.
        return memo.getId();//update코드가 memo자바 클래스 안에 안 만들어져 있기에 memo자바 클래스 안에 만들러 가야함.
   }
